<?php 
	...
	if(isset($_POST['foget_password_button'])){
		...
		$url="http://localhost/E-commerce/reset_password.php?selector=".$selector."&validator=".bin2hex($token);
		...
		if(empty($email)){
			...
			header("Location:../fogot_password.php?error=emptyfields");
			exit(); 
		}
		else if(!filter_var($email,FILTER_VALIDATE_EMAIL)){
			header("Location:../fogot_password.php?error=invalidemail");
			exit();
		}
		else{
			...
			if($result->num_rows>0){
				...
				header("Location:../fogot_password.php?reset=successful&url=".$url);
				...
				header("Location:../fogot_password.php?error=usernotfound");
			} 
		}
	}
	else{
		header("Location:../fogot_password.php");
		exit(); 
	}
